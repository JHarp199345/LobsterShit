...
function createCommandHandlers(context) {
	...
	const sendMessage = async (text) => {
		...
		try {
			...
			chatLog.addSystem(`event: send started runId=${runId.slice(0, 8)}`);
			setActivityStatus("waiting");
			tui.requestRender();
		} catch (err) {
			if (state.activeChatRunId) forgetLocalRunId?.(state.activeChatRunId);
			state.activeChatRunId = null;
			chatLog.addSystem(`event: send failed — ${String(err)}`);
...
function createEventHandlers(context) {
	...
	const handleChatEvent = (payload) => {
		...
		if (evt.state === "aborted") {
			const wasActiveRun = state.activeChatRunId === evt.runId;
			chatLog.addSystem(`event: run aborted runId=${String(evt.runId).slice(0, 8)}`);
			terminateRun({
				runId: evt.runId,
				wasActiveRun,
				status: "aborted"
		...
		if (evt.state === "error") {
			const wasActiveRun = state.activeChatRunId === evt.runId;
			const reason = evt.errorMessage ?? "unknown";
			chatLog.addSystem(`event: run failed — ${reason}`);
			terminateRun({
				runId: evt.runId,
				wasActiveRun,
				status: "error"
	...
	const handleAgentEvent = (payload) => {
		...
		if (evt.stream === "lifecycle") {
			...
			if (phase === "start") {
				chatLog.addSystem(`event: agent run started runId=${String(evt.runId).slice(0, 8)}`);
				setActivityStatus("running");
			}
			if (phase === "end") {
				chatLog.addSystem(`event: agent run completed runId=${String(evt.runId).slice(0, 8)}`);
				setActivityStatus("idle");
			}
			if (phase === "error") {
				const err = evt.data?.error ?? "unknown";
				chatLog.addSystem(`event: agent run failed — ${String(err)}`);
				setActivityStatus("error");
			}
			tui.requestRender();
		}
...
function createSessionActions(context) {
	...
	const loadHistory = async () => {
		try {
			...
			chatLog.addSystem(`event: history loaded`);
		} catch (err) {
			chatLog.addSystem(`event: history failed — ${String(err)}`);
		}
		await refreshSessionInfo();
		tui.requestRender();
	};
...
async function runTui(opts) {
	...
	client.onConnected = () => {
		...
		chatLog.addSystem(`event: ${reconnected ? "reconnected" : "connected"}`);
	...
	client.onDisconnected = (reason) => {
		chatLog.addSystem(`event: disconnected — ${reason?.trim() || "closed"}`);
		isConnected = false;
