 ...
 * Subscribes to exec.approval.requested events and prompts for y/n in the CLI.
 * Run in a split pane alongside the TUI for instant approval feedback.
 *
 * Usage: node scripts/approval-watcher.mjs
 * Or:    pnpm run approval-watch (from openclaw-core)
 */

import { createInterface } from "node:readline";
import { randomUUID } from "node:crypto";
import { t as GatewayClient, $t as loadOrCreateDeviceIdentity, At as PROTOCOL_VERSION } from "../dist/client-EwxHy0Jk.js";
import { t as buildGatewayConnectionDetails, o as resolveExplicitGatewayAuth } from "../dist/call-Dx-c0m2G.js";
import { Rt as loadConfig } from "../dist/model-selection-J6oFwo9y.js";
import { h as GATEWAY_CLIENT_NAMES, m as GATEWAY_CLIENT_MODES } from "../dist/message-channel-BFAJAoI_.js";
import { t as resolveGatewayCredentialsFromConfig } from "../dist/credentials-xxcK5iF5.js";
import { g as resolveStateDir, o as resolveConfigPath } from "../dist/paths-B4BZAPZh.js";
...
function main() {
  ...
  const client = new GatewayClient({
    ...
    onConnectError: (err) => {
      ...
      console.error(`[approval-watcher] Connect failed: ${msg}`);
      if (isRefused) {
        console.error("");
        console.error("Gateway not reachable. Start it first:");
        console.error("  • Run Option 4 (TUI) in Command Center — it may start the gateway");
        console.error("  • Or: openclaw gateway start");
        console.error("");
      }
      process.exit(1);
    },
